...
*** Extra commands
   ...
   Adds command ~gc~ to comment out code.
   ...
   Adds ~surround~ as a text object with ~s~. For example ~cs([~ to replace the
   ...
   Adds command to align text based on a character with ~gl~. For example ~glip=~ to align all lines
   in a paragraph on the equal sign.
...
** Org Mode
   ...
   Allows running python inside org. write "#+begin_src python :results output" to start a code
   block that can output results.
